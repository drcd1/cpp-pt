#ifndef CPPPT_PATHTRACER_MLT_MIS
#define CPPPT_PATHTRACER_MLT_MIS
...
class PathtracerMLT_MIS : public Renderer{
    int samples;
            ...
            if(!intersected){
                ...
                if(bsdf->is_emitter()){
                    if(sampled_delta){
                        ...
                        col = col + mul*bsdf->emit(ray.d*(-1.0),intersection)*(1.0-weight);
                    }
                    break;
                }
                sampled_delta = false;
                ...
                if(!sample.delta) {
                    ...
                    if(!scene.primitive->intersect_any(shadow_ray)){
                        ...
                        col = col + mul*bsdf->eval(
                            ...
                            shadow_ray.d,
                            intersection
                        )*light_sample.intensity/(light_sample.pdf*r*r)*weight;
                    }
                    }
                } else {
                    sampled_delta = true;
...
public:
    PathtracerMLT_MIS(const RenderSettings& rs): samples(rs.spp) {}
