...
bool LoadTexture(const RgbImage* image, MyTextureData* tex_data)
{

    // Calculate allocation size (in number of bytes)
    //note we'll store it in srgb
    ...
    for(int i = 0; i<image->Width(); i++){
        for(int j = 0; j<image->height(); j++){
            int idx = j+i*image->height;
            image_data[idx*4]= char(cpppt::linear2srgb(image->at(i,j).r)*255.0);
            image_data[idx*4+1]= char(cpppt::linear2srgb(image->at(i,j).g)*255.0);
            image_data[idx*4+2]= char(cpppt::linear2srgb(image->at(i,j).b)*255.0);
